...
var interrupt_groups = {
	...
	"skills":["section","perforation trifecta","cleave","battlecry","scream"],
...
func setSkillAnimation(skill_name:String)->void:
	...
	# --------------------------------------------------
	# Skill does not exist in the animation database.
	# Nothing to do.
	# --------------------------------------------------
	if !skill_animations.has(skill_name):
		return

	# Get animation mappings for this skill.
	# Example:
	# {WeaponMode.SWORD:"1h_Slice", WeaponMode.DUAL:"dual_Slice"}
	var skill_data=skill_animations[skill_name]

	# Animation that will be selected.
	var new_anim=""

	# --------------------------------------------------
	# Check whether this skill supports the currently
	# equipped weapon mode.
	# --------------------------------------------------
	if skill_data.has(weapons):
		...
		# --------------------------------------------------
		# Current weapon mode has no animation assigned.
		#
		# Example:
		# Skill only supports swords but player is using
		# a shield or two-handed weapon.
		#
		# Cancel the skill and refund any spent resources.
		# --------------------------------------------------
		anim_locks[skill_name]=false
		current_skill="none"

		# Refund energy and remove cooldown.
	...
	# Extra safety check.
	#
	# Even if an entry exists in the dictionary, the
	# assigned animation might be an empty string.
	#
	# Treat that as an unsupported weapon mode and
	# refund the skill.
	# --------------------------------------------------
	if new_anim=="":
		...
		# Refund energy and cooldown.
		skillbar.reimburseSkill(skill_name)

		return

	# --------------------------------------------------
	# Prevent unnecessary AnimationTree refreshes if
	# the requested animation is already active.
	# --------------------------------------------------
	if new_anim==last_skill_animation:
		return

	# Store the animation as the last one used.
	last_skill_animation=new_anim

	# Assign animation to the Skill node.
	skill_anim.animation=new_anim

	# --------------------------------------------------
	# Force AnimationTree to refresh and pick up the
	# newly assigned animation.
	# --------------------------------------------------
