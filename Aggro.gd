...
func update_stuck_aggro_decay(mob):
	if Engine.get_physics_frames() % 32 == 0:
		var last_position = mob.get_meta(
			"last_stuck_position"
		) if mob.has_meta(
			"last_stuck_position"
		) else mob.global_transform.origin

		var moved_distance = mob.global_transform.origin.distance_to(
			last_position
		)
		mob.set_meta(
			"last_stuck_position",
